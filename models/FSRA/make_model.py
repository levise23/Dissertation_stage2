...
class build_transformer(nn.Module):
    def __init__(self, opt, num_classes, block=4, return_f=False):
        ...
        if self.text_fuse:
            self.text_proj = nn.Linear(self.text_dim, self.in_planes)
            self.text_ln = nn.LayerNorm(self.in_planes)
            self.cross_attn = nn.ModuleList([
                nn.MultiheadAttention(self.in_planes, num_heads=8, batch_first=True)
                for _ in range(max(1, self.text_layers))
            ])
    ...
    def forward(self, x, text_feat=None):
        ...
        if self.text_fuse and text_feat is not None:
            # text_feat: (B, text_dim) -> (B, 1, C)
            text_feat = text_feat.to(features.dtype)
            text_token = self.text_ln(self.text_proj(text_feat)).unsqueeze(1)
            patch_tokens = features[:, 1:, :]
            for attn in self.cross_attn:
                attn_out, _ = attn(patch_tokens, text_token, text_token, need_weights=False)
                patch_tokens = patch_tokens + torch.tanh(self.text_gate) * attn_out
            features = torch.cat([features[:, :1, :], patch_tokens], dim=1)
